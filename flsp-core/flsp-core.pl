...
sub query_bandsintown_artist($artist) {
    ...
    my $url =
"https://rest.bandsintown.com/artists/$url_encoded_artist?app_id=$bit_api_key";
...
sub query_bandsintown_shows($artist) {
    my $url_encoded_artist = url_encode_utf8($artist);
    my $url =
"https://rest.bandsintown.com/artists/$url_encoded_artist/events?app_id=$bit_api_key";
...
"INSERT INTO artists (name, bit_id, url, image_url, thumb_url, facebook_page_url, mbid, updated) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())"
    );
    $rv = $ins_stmt->execute(
        $artist_info->{name},      $artist_info->{id},
        $artist_info->{url},       $artist_info->{image_url},
        $artist_info->{thumb_url}, $artist_info->{facebook_page_url},
        $artist_info->{mbid},
    );
    unless ($rv) {
    ...
    my ($artists_table_id) =
      $dbh->last_insert_id( undef, undef, "artists", undef );

    ### mark artist as initialized
    my $upd_stmt = $dbh->prepare(
"UPDATE followed_artists SET first_query_complete = TRUE, artist_id = ? WHERE id = ?"
    );
...
sub update_showlist ( $dbh, $shows ) {
    ...
    my $event_ids_array =
      $dbh->selectcol_arrayref("SELECT event_id FROM events");
    my %event_ids;
    ...
    for my $show (@$shows) {
        ...
        my $ins_stmt = $dbh->prepare(
"INSERT INTO events (artist_id, event_id, url, on_sale_datetime, datetime, description, title, venue, country, region, city, lineup, added) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW() )"
        );
        my $rv = $ins_stmt->execute(
            $show->{artist_id},        $show->{id},
            $show->{url},              $show->{on_sale_datetime},
            $show->{datetime},         $show->{description},
            $show->{title},            encode_json( $show->{venue} ),
            $show->{venue}->{country}, $show->{venue}->{region},
            $show->{venue}->{city},    $show->{lineup},
        );
...
"SELECT id, name FROM followed_artists WHERE first_query_complete IS FALSE AND deleted IS FALSE"
    );
...
sub query_all_artists($dbh) {
    ...
    my $rows = $dbh->selectall_arrayref(
"SELECT artist_id FROM followed_artists WHERE first_query_complete IS TRUE AND deleted IS FALSE"
    );
    ...
    for my $r (@$rows) {
        ...
        # get artist name
        my $sel_stmt =
          $dbh->prepare("SELECT name FROM artists WHERE bit_id = ?");
        $sel_stmt->execute($id);
...
$bit_api_key = $ENV{"BANDSINTOWN_API_KEY"}
  || die "Must define BANDSINTOWN_API_KEY env var!";
