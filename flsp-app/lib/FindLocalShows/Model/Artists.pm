...
sub add_artist ( $self, $artist ) {
    ...
    my $sel_stmt =
      $self->{dbh}->prepare("SELECT id FROM followed_artists WHERE name = ?");
    ...
    if (@row) {
        ...
        # undelete if present
        my $upd_stmt = $self->{dbh}
          ->prepare("UPDATE followed_artists SET deleted = FALSE WHERE id = ?");
...
sub remove_artist ( $self, $id ) {
    ...
    my $sel_stmt =
      $self->{dbh}->prepare("SELECT name FROM followed_artists WHERE id = ?");
    $sel_stmt->execute($id);
    ...
    if ($row) {
        ...
        # delete
        my $upd_stmt = $self->{dbh}
          ->prepare("UPDATE followed_artists SET deleted = TRUE WHERE id = ?");
        $upd_stmt->execute($id);
        return $row->[0];
    }
