...
sub get_secret($self, $dbh) {
	...
	$self->app->log->debug("trying to select secret from db: row: " . Dumper($row));
	die $dbh->errstr if ( defined $dbh->errstr );  # encountered database error
	if ( defined $row ) {
		$self->app->log->debug("using app secret from database");
		return $row->[0];
	}

	# try to obtain secret from environment
	$self->app->log->debug("using app secret from environment");
